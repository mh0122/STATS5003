...
```{r}
# datanumeric <- as_tibble(data)
# datanumeric
```

```{r}

datamatrix <- data.matrix(data, rownames.force = NA) #converting df to matrix
PCAdatamatrix <- datamatrix[,-1] #dropping cvd column

par(mfrow=c(1,2)) # to plot multiple charts
...
```
Applying PCA on the chosen 21 variables isn't a good idea since they are all contributing actively to explain the variance. Apart from one component that contributes 16%; the rest are all single figure percentage contributors (less than 10%)
...
ggplot(cv_glm_scores, aes(x = ind, y = value))  + 
  ...
  labs(title = "Logistic Regression Validation Results",
              ...
              x = "Performance MeasuresYe", y = "Value",
  )
