...
We evaluate the model performance on test data by finding the roc auc score. We see that the auc score on the original test set is 0.849, and 0.850 on the various sampling techniques to the data.
...
# create full model 
cv_glm_fit <- glm(HeartDiseaseorAttack ~ ., data = data, family = 'binomial')
cv_glm_predicted <- as.factor(ifelse(cv_glm_fit$fitted.values > 0.1, 1, 0))
confusionMatrix(data = cv_glm_predicted, reference = data$HeartDiseaseorAttack, positive = "1")
roc.curve(data$HeartDiseaseorAttack, cv_glm_predicted, plotit = TRUE)
...
Logisitic Regression avg. Sensitivity = 79% and Specificity = 75% with 10-fold cross validation 